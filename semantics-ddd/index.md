---
title: "[]{#_82rr9yl45cuy .anchor}Domain Driven Design and Ontologies"
---
...
Evans in 2003 focusing on modeling software based on the input from
...
-   a [[Bounded
    ...
    requirements, processes and data models) associated with a Domain or
    a Sub-Domain;
...
disambiguating terms. Terms are identified by URIs prefixed with the
...
**RDF focuses on knowledge and semantic relations**, for example, from
...
(rdfs:Class) to \"divide resources into groups. To support knowledge
mapping, the **RDF data model is based on graphs** .
...
from Greek philosophy. This means that ontologies are focused on
describing knowledge, regardless of service implementations. Technical
experts are expected to use this formal knowledge to support the
creation of different kinds of digital services: from mobile
applications to backend microservices.

From a Domain-Driven perspective, **semantic web experts focus on the
domain, while technical experts focus on dynamic bounded contexts.** RDF
is thus a great tool to support the definition of an Ubiquitous Language
and the formalization of the entities belonging to a Domain Model, but
ontologies are focused on the problem space. **To fit the solution
...
use semantic technologies such as RDF and SHACL for data modeling. It is
important that, inside a domain, knowledge is formally described using
...
service-specific solution space, but it is possible to acknowledge this
fact and use RDF to design specific application profiles that are not
ontologies, and that are used for data definition.

The main problem is that semantic technologies are not optimized to
constrain syntax:
...
-   tools like Shape Constraint Language (SHACL) can address some with
    syntax use cases, but are considered overly complex and
...
While ontologies are great tools to map the knowledge inside a domain,
...
This section highlights the difference in DD between public and private
services

## What does Domain mean for public services?

**The Domain in the private sector is defined by customer expectation**
and can evolve accordingly. Unsatisfied users can change providers
depending on the degree of competition in the specific business areas.
Regulation and compliance are often non-functional requirements: their
impacts on usability are mitigated by designers being free to adapt
workflows and processes internally in order to accommodate both
functional and non-functional aspects.

**In the public sector** citizen expectations in a specific Domain (e.g.
healthcare) are not directly addressed by services. Instead,
**regulation provides the solution space via bureaucratic procedures**
that define the Domain Model and the Bounded Contexts; this is usually
done accommodating the needs of both the citizens and the organizational
structures of the agencies and, for new services, this activity is done
*a priori*, since experimenting a regulatory framework is seldom
feasible. Digital public services\' requirements are provided in terms
of regulation and bureaucratic procedures: this means that their problem
space is different from the citizen\'s one.

Moreover, while for the citizens the problem space remains the same
(e.g. to get medication), regulatory changes might require a complete
redesign of the existing services. Shortly, public sector digital
...
-   in practice, digital service implementers consider the regulation
    and all the bureaucratic procedures as the Domain, and the problem
...
This means that digital public services outcome may diverge from citizen
expectation: laws and organizational procedures are both domain and
model, problem and solution. This has many consequences, including:
...
-   costly and ineffective digital public services when bureaucratic
    ...
    time-consuming this kind of issues impacts for years;

-   [[bureaucratic procedures are tightly coupled with the specific
    organizational
    structures](https://onezero.medium.com/our-kill-it-with-fire-moment-f900aaabd743)
    of the agencies: this may impact beyond what is necessary on the
    service design. Moreover, since public sector organization can
    ...
    can be disrupted without any new requirement change coming from
    citizens;

-   regulation changes are not foreseeable (e.g. due to sentences,
    Community or Federal Laws), and it is hard to assess their impacts
    on existing services: for example such changes could disrupt the
    Design Model of deployed services, imposing changes that spillover
    between established Bounded Contexts. These changes are usually more
    frequent in large countries, where there are more use cases to
    address; or in countries with a federal organization where similar
    use cases are addressed in different ways.

-   different jurisdictions have different regulations, so different
    solutions to similar problem spaces;

-   agencies exposing APIs intended for general/cross-border consumption
    have a hard time to identify usability requirements.

The following image shows two digital services relying on a third-party
API provided by the Population Registry: the architecture of this API
(that pre-dates the services) depends by its solution space ([Domain
Model] and [Bounded Contexts]) and will affect the usability and the
...
As a closing remark, it is important to say that, in the government
sector, it is normal that regulation is the primary source for the
solution space. Stakeholders need to acknowledge that and mitigate the
associated issues.

## The solution space in digital public services

This section shows the elements that affect the definition of the
solution space in digital public services, and which strategies can be
followed to mitigate the risks related to the different perspectives
between consumers\' and IT architects\' goals illustrated in [*[What
does Domain mean for public
services?]*](#what-does-domain-mean-for-public-services).

From the IT architect perspective, the problem space is now the
regulation. Semantic experts work to align ontologies to the regulation:
...
-   functional requirements based on regulation and procedures that can
    constrain the Domain Model (e.g. suboptimal grouping of operations
    inherited from analogic procedures, convoluted data models based on
    paper forms, ...);
-   non-functional requirements like latency, durability,
    confidentiality, integrity and availability;
-   friction in addressing issues regulation and procedures.

When working inside a single domain, it is possible to mitigate some of
these problems with [[digital
...
at legal and organizational level (e.g. ensuring that regulation and
procedures do not hinder the creation of digital services and avoid laws
providing too much procedural or technical details that should instead
be delegated to the implementation) or adopting a [[user-centric
